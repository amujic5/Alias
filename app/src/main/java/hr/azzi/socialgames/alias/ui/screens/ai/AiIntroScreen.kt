...
fun AiIntroScreen(challenge: AIChallenge, onPlay: () -> Unit, onClose: () -> Unit) {
    BrandBackground {
        Column(Modifier.fillMaxSize().systemBarsPadding().padding(24.dp),
            ...
            DisplayText(stringResource(R.string.ai_challenges_you, challenge.creatorName), 28)
            ...
            CCard(Modifier.fillMaxWidth()) {
                Column(horizontalAlignment = Alignment.CenterHorizontally, modifier = Modifier.fillMaxWidth()) {
                    ...
                    Text(stringResource(R.string.ai_deck_info, challenge.words.size, challenge.totalSeconds, challenge.aiLanguage.display),
                        color = Alias.textSecondary, fontFamily = Alias.body, fontSize = 14.sp, textAlign = TextAlign.Center)
                    Spacer(Modifier.height(14.dp))
                    Text(stringResource(R.string.ai_beat_score, challenge.creatorScore), color = Alias.blue600,
                        fontFamily = Alias.display, fontWeight = androidx.compose.ui.text.font.FontWeight.Bold, fontSize = 16.sp)
                }
            }
            Spacer(Modifier.weight(1f))
            PillButton(stringResource(R.string.ai_play_challenge), PillKind.Primary, onClick = onPlay)
            Spacer(Modifier.height(12.dp))
            PillButton(stringResource(R.string.ai_not_now), PillKind.Light, onClick = onClose)
        }
    }
}
